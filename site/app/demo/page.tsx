  ...
  return (
    <SiteShell current="demo">
      <section className="page-hero compact-hero section-dark">
        <div className="shell split-section"><div><p className="eyebrow">A real dashboard, frozen in time</p><h1>{snapshot.plan?.title ?? "No active plan in the snapshot"}</h1><p>{snapshot.plan?.summary ?? "No saved plan to show yet."}</p><div className="snapshot-notice"><strong>Nothing staged.</strong> This is a saved copy of a real learner’s plan. No database, no AI, nothing running behind it. Just the receipts.</div></div><Owl pose="planning" alt="Hoolio planning milestones" /></div>
      </section>
      <section className="dashboard section-dark">
        <div className="shell">
          ...
          <div className="transcript-preview"><div><p className="eyebrow">Latest evidence</p><h2>The learner’s work stays attached.</h2><p>{snapshot.attempts.length} attempts and {snapshot.hints.length} earned hints ship inside this saved copy. The plan never floats free of the work.</p></div><div className="preview-messages">{readerMessages.slice(-2).map((message) => <blockquote key={message.id}><span>{message.label}</span><p>{message.body}</p></blockquote>)}</div><a className="button button-secondary" href={withBase(`/t/${transcriptSlug}/`)}>Read all messages →</a></div>
        </div>
      </section>
    </SiteShell>
  );
