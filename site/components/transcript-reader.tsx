  ...
  return (
    <div className="reader-layout">
      <aside className="reader-tools" aria-label="Transcript controls">
        <div className="mode-tabs"><span className="active" aria-current="true">Reader mode</span><span aria-disabled="true">Replay · Pass B</span></div>
        ...
        <p>{filtered.length} of {messages.length} persisted messages shown.</p>
      ...
      <div className="reader-stream" aria-live="polite">
        <aside className="reader-message snapshot-boundary marker-refusal" id="boundary-refusal"><div className="margin-marker">Refusal unavailable</div><header><span>Snapshot boundary</span><time>No persisted event</time></header><p>The current database export contains no refusal transcript entry, so this reader does not manufacture one. The persisted conversation begins below.</p></aside>
        ...
        {!filtered.length ? <p className="empty-search">No persisted message matches “{query}”.</p> : null}
      </div>
    </div>
  );
}
